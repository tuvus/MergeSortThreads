...
/**
 * MergeSortThreadsDivideReference uses Arrays.Sort() instead of the regular mergesort to do the main sorting.
 * MergeSortThreadsDivideReference is an instantiated class that sorts an array given in its constructor during its life.
 * MergeSortThreadsDivideReference uses the classical MergeSort algorithm using multithreading.
 * MergeSortThreadsDivideReference splits the array into x sections where x is the amount of threads to use.
 ...
 * To ensure that the array is sorted call .complete() on
 *
 * @param <E> the type to be sorted
 * @author Oskar
 */
public class MergeSortThreadsDivideReference<E extends Comparable<? super E>> {
    private E[] array;
    private E[] copy;
     ...
     */
    private MergeSortThreadsDivideReference() {
    }

    /**
     * Initialises MergeSortThreadsDivideReference with an array.
     * The number of threads that will be used is equal to Runtime.getRuntime().availableProcessors() - 1.
     *
     * @param array the array to sort
     */
    public MergeSortThreadsDivideReference(E[] array) {
        this(array, Runtime.getRuntime().availableProcessors() - 1);
    }

    /**
     * Initialises MergeSortThreadsDivideReference with an array and a desired threadCount.
     *
     * @param array       the array to sort
     * @param threadCount the number of threads that should be used while sorting
     */
    public MergeSortThreadsDivideReference(E[] array, int threadCount) {
        this.array = array;
    ...
    public void start() {
        if (completed)
            throw new IllegalStateException("Trying to start MergeSortThreadsDivideReference after the array was already finished." +
        ...
        for (int i = 0; i < sections.size(); i++) {
            if (sections.get(i).mergeSortThread.getState() != Thread.State.NEW)
                throw new IllegalStateException("Trying to run MergeSortThreadsDivideReference after calling start. " +
    ...
    public void run() {
        if (completed)
            throw new IllegalStateException("Trying to run MergeSortThreadsDivideReference after the array was already finished." +
    ...
    private class MergeSortThread extends Thread {
         ...
         * If it is the last thread running it will set the MergeSortThreadsDivideReference as sorted
        ...
        public void run() {
            ...
            if (section.outputArray)
                Arrays.sort(array, section.lowerIndex, section.upperIndex + 1);
            else
                Arrays.sort(copy, section.lowerIndex, section.upperIndex + 1);
